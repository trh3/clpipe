...
class Job:
    ...
    def git_sandwich(self):
        ...
        suffix = " && datalad push -d {tmp_dir} --to origin && git -C {target_dir} merge -m \"{merge_message}\" {jobid}".format(target_dir = self.target_dataset, tmp_dir = self.tmp_dir, jobid = self.jobID, merge_message = "TEMPORARY MERGE MESSAGE for Job " + self.jobID)
